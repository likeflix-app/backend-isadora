...
if (!DATABASE_URL) {
  ...
  console.log('');
  console.log('Usage:');
  console.log('  node cleanup-pending.js "postgresql://user:password@host:port/database"');
...
async function cleanupPendingApplications() {
  try {
    console.log('🔄 Starting cleanup of pending applications...');
    ...
    const pendingApplications = await db.any(
    ...
    console.log(`📋 Found ${pendingApplications.length} pending application(s)`);
